...
import { 
  ShieldCheck, AlertCircle, Coins, CreditCard, Smartphone, Copy, Check 
} from "lucide-react";
...
export default function WalletClient() {
  ...
  // FUNCIÓN RESTAURADA CORRECTAMENTE
               ...
               {activeTab === 'deposit' ? (
                   <>  
                       <div className="grid grid-cols-2 gap-2 mb-4">
                          ...
                          <button onClick={() => setMethod('crypto')} className={`p-3 rounded-xl border flex flex-col items-center gap-2 transition-all ${method === 'crypto' ? 'border-purple-500 bg-purple-500/10 text-white' : 'border-white/10 bg-black/20 text-zinc-500 hover:bg-white/5'}`}>
                             <Coins size={16} />
                          </button>
                       </div>
                       ...
                       {method === 'oxxo' && (
                           <div className="bg-black/40 border border-white/10 rounded-xl p-4 animate-fade-in">
                               ...
                               <button onClick={createDeposit} disabled={creating} className="w-full py-3 rounded-xl font-bold bg-chido-gold text-black hover:brightness-110 shadow-lg">{creating ? "Generando..." : "GENERAR FICHA OXXO"}</button>
                           </div>
                       )}
                       ...
                       {method === 'card' && (
                           <div className="animate-fade-in">
                               ...
                               <button onClick={createDeposit} disabled={creating} className="w-full py-4 rounded-xl font-black text-lg shadow-lg transition-all mt-4 bg-gradient-to-r from-chido-pink to-red-600 text-white hover:brightness-110">{creating ? "Procesando..." : "PAGAR CON TARJETA"}</button>
                           </div>
                       )}
                   </>
               ) : (
                   <>
                       ...
                       <div className="space-y-2">
                           <label className="text-[10px] font-bold text-zinc-500 uppercase ml-2">CLABE</label>
                       ...
                       <button onClick={handleWithdraw} disabled={creating} className="w-full py-4 rounded-xl font-black text-lg shadow-lg transition-all mt-6 bg-white text-black hover:scale-[1.02]">{creating ? "Solicitando..." : "RETIRAR MIS GANANCIAS"}</button>
                   </>
               )}
               {msg && (
