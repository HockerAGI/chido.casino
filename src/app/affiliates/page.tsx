  ...
  if (loading) {
    return (
      <div className="min-h-screen pb-20 animate-fade-in flex items-center justify-center text-white/70 gap-2">
        <Loader2 className="animate-spin" size={18} /> Cargando tu panel de compas…
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen pb-20 animate-fade-in max-w-4xl mx-auto p-6">
        <Card className="bg-[#1A1A1D] border-white/10 p-6 rounded-2xl">
          <div className="text-xl font-black">Programa de Compas</div>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p className="text-zinc-400 text-sm md:text-base font-medium">
              Tu código está activo. Comparte tu link y gana una comisión de cada apuesta que hagan tus referidos.
            </p>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-8 relative z-10 space-y-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-[#1A1A1D] border-white/5 p-5 rounded-2xl shadow-xl">
            ...
            <div className="text-3xl font-black text-white/50" title="Próximamente">{s.clicks}</div>
          </Card>
          ...
          {/* Primeros Depósitos (Placeholder) */}
          <Card className="bg-[#1A1A1D] border-white/5 p-5 rounded-2xl shadow-xl">
            <div className="flex items-center justify-between mb-2">
              <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">1er Depósito</div>
              <div className="p-1.5 bg-white/5 rounded-lg text-zinc-400"><Users size={16} /></div>
            </div>
            <div className="text-3xl font-black text-white/50" title="Próximamente">{s.firstDeposits}</div>
          </Card>
          ...
          <Card className="bg-[#1A1A1D] border-white/5 p-5 rounded-2xl shadow-xl relative overflow-hidden">
            ...
            <div className="text-3xl font-black text-[#00F0FF] relative z-10 drop-shadow-md">
              {totalCommission.toFixed(2)} <span className="text-lg text-white/50">{CURRENCY_SYMBOL}</span>
            </div>
          </Card>
        </div>
        ...
        <Card className="bg-[#1A1A1D] border-white/5 p-6 rounded-2xl shadow-xl">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <div className="text-xs text-zinc-500 uppercase font-bold tracking-widest mb-1">Tu link de compa</div>
        ...
        <Card className="bg-[#121214] border-white/5 p-6 rounded-2xl">
          <div className="text-lg font-black mb-3">Últimas comisiones ganadas</div>
                ...
                <div key={i} className="flex items-center justify-between rounded-xl bg-black/30 border border-white/10 p-3">
                  <div>
                    ...
                    <div className="text-xs text-white/50">
                      {c.created_at ? new Date(c.created_at).toLocaleString() : "—"} • <span className="capitalize">{c.status}</span>
                    </div>
                  </div>
                  <div className="font-mono text-sm tabular-nums text-[#32CD32]">
                    +{Number(c.amount || 0).toFixed(2)} {CURRENCY_SYMBOL}
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-sm text-white/60">Cuando tus compas jueguen, aquí verás tus ganancias.</div>
          )}
        </Card>
      </div>
    </div>
