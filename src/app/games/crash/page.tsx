...
export default function CrashGamePage() {
  ...
  const startGame = () => {
      ...
      // Simular punto de crash aleatorio (En prod vendría del backend)
      crashPointRef.current = 1 + Math.random() * 5; // Crash entre 1.00x y 6.00x
      
      intervalRef.current = setInterval(() => {
          setMultiplier(prev => {
              const next = prev + 0.01 + (prev * 0.004); // Curva exponencial
  ...
  const cashOut = () => {
      ...
      // Aquí enviarías la petición al backend para acreditar ganancia
  };
  ...
  const getStatusColor = () => {
      ...
      if (gameStatus === 'running') return 'text-white';
      return 'text-white';
  };
      ...
      <div className="grid lg:grid-cols-[1fr_350px] gap-6 h-[calc(100vh-140px)] min-h-[600px] animate-fade-in px-4 pb-4">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex-1 bg-zinc-900 rounded-3xl border border-white/5 relative overflow-hidden flex flex-col items-center justify-center group shadow-2xl">
            ...
            <div className={`relative z-10 text-center transition-transform duration-100 ${gameStatus === 'crashed' ? 'shake' : ''}`}>
              ...
              <div className="flex justify-center mt-4">
                  ...
                  {gameStatus === 'idle' && <span className="bg-white/10 text-zinc-400 px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest">Listo para despegar</span>}
              </div>
            </div>

            {/* Visualización Simple de Línea (SVG) */}
            {gameStatus === 'running' && (
          ...
          <div className="bg-zinc-900 rounded-3xl p-6 border border-white/5 flex flex-col md:flex-row gap-6 items-center shadow-lg relative overflow-hidden">
             ...
             <div className="flex-1 w-full space-y-2">
                 ...
                 <div className="relative">
                     ...
                     <input 
                        ...
                        className="w-full bg-black/50 border border-white/10 rounded-xl py-4 pl-8 pr-4 text-white font-black text-2xl focus:border-chido-cyan outline-none transition-all" 
                     />
                 </div>
                 <div className="flex gap-2">
                    {["10", "50", "100", "MAX"].map(val => (
                        <button key={val} onClick={() => val !== "MAX" && setBetAmount(val)} className="flex-1 bg-white/5 hover:bg-white/10 rounded-lg py-1 text-xs font-bold text-zinc-400 transition-colors">{val}</button>
                    ))}
                 </div>
             </div>
             ...
             {gameStatus === 'running' ? (
                 <button 
                    onClick={cashOut} 
                    className="w-full md:w-auto md:px-12 h-20 bg-chido-green text-black font-black text-2xl rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-[0_0_30px_rgba(50,205,50,0.4)] flex flex-col items-center justify-center leading-none"
                 >
                    <span>RETIRAR</span>
        ...
        <div className="hidden lg:flex flex-col bg-zinc-900 rounded-3xl border border-white/5 overflow-hidden">
          <div className="p-5 border-b border-white/5 flex items-center justify-between">
              ...
              <span className="w-2 h-2 rounded-full bg-chido-green animate-pulse shadow-[0_0_10px_#32CD32]"></span>
          </div>
