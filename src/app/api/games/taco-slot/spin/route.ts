...
function calcMultiplier(reels: { key: SymbolKey }[]) {
  ...
  return first === second || second === third || first === third ? PAIR_MULTIPLIER : 0;
}
...
export async function POST(req: Request) {
  try {
    ...
    const { data: nonceData, error: nonceError } = await supabaseAdmin.rpc("next_slot_nonce");
    if (nonceError) {
      console.error("Slot nonce generation failed", nonceError);
      return NextResponse.json({ ok: false, error: "NONCE_FAILED" }, { status: 500 });
    }
    const nonce = Number(nonceData);
    if (!Number.isSafeInteger(nonce) || nonce <= 0) {
      return NextResponse.json({ ok: false, error: "NONCE_INVALID" }, { status: 500 });
    }
    ...
    const reels = [0, 1, 2].map((round) => pickWeighted(serverSeed, clientSeed, nonce, round));
    const multiplier = calcMultiplier(reels);
    const payout = multiplier > 0 ? roundMoney(bet * multiplier) : 0;

    const { data: settlement, error: settlementError } = await supabaseAdmin.rpc("casino_settle_taco_slot", {
      ...
      p_nonce: nonce,
      p_metadata: { pair_multiplier: PAIR_MULTIPLIER, expected_rtp: 0.947376 },
    });
    ...
    return NextResponse.json({
      ...
      fair: { serverSeedHash: serverSeedHashHex, serverSeed, clientSeed, nonce },
