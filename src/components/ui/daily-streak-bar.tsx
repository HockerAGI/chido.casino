...
import { Flame, Gift, CheckCircle2, X, Loader2 } from "lucide-react";
...
export function DailyStreakBar() {
  ...
  const todayReward = useMemo(() => DAY_REWARDS[Math.min(streak, 6)], [streak]);
  ...
  const claim = async () => {
    ...
    try {
      const res = await fetch("/api/promos/claim-daily-streak", {
        method: "POST",
        headers: { "content-type": "application/json" },
      });

      const data = await safeJson<ClaimResponse>(res);

      if (!res.ok || !data.ok) {
        throw new Error(data?.error || data?.message || "No se pudo reclamar la racha.");
      }

      const nextStreak = Math.min(7, Number(data.streak || streak + 1));
      const today = new Date().toDateString();

      setStreak(nextStreak);
      setClaimed(true);
      setShowModal(false);
      setFeedback(data.message || `Recompensa acreditada: ${data.awarded ?? 0} MXN`);

      try {
        localStorage.setItem("chido_streak", JSON.stringify({ streak: nextStreak, lastClaim: today }));
      } catch {
        // ignore
      }
      ...
      setFeedback(e?.message || "No se pudo reclamar la racha.");
    } finally {
      setClaiming(false);
    }
  };
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-sm font-black text-white">Racha diaria activa</span>
              <span className="text-[10px] font-bold text-[#FF5E00] bg-[#FF5E00]/10 border border-[#FF5E00]/20 rounded-full px-2 py-0.5">
                Bono
              </span>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`flex-1 rounded-lg py-1 text-center transition-all ${
                        ...
                        ? "bg-[#FF5E00]/30 border border-[#FF5E00]/40"
                        : isToday
                          ? "bg-[#FFD700]/15 border border-[#FFD700]/40 animate-pulse"
                          : "bg-white/5 border border-white/5"
                    }`}
                  >
                    <div className="text-[9px] font-black leading-none">
                      {done ? (
                        <CheckCircle2 size={9} className="text-[#FF5E00] mx-auto" />
                      ) : isToday ? (
                        <span className="text-[#FFD700]">HOY</span>
                      ) : (
                    ...
                    <div
                      className={`text-[8px] font-bold mt-0.5 leading-none ${
                        done ? "text-[#FF5E00]" : isToday ? "text-[#FFD700]" : "text-white/20"
                      }`}
                    >
                      {d.reward}
                    </div>
          ...
          <div className="shrink-0 flex flex-col gap-1.5 items-end">
            ...
            <button onClick={dismiss} className="text-[10px] text-white/20 hover:text-white/50 transition-colors">
              cerrar
            </button>
          </div>
        </div>

        {feedback && <div className="relative mt-3 text-[11px] text-white/60">{feedback}</div>}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-sm rounded-3xl border border-white/10 bg-gradient-to-b from-[#1a0a00] to-[#0d0810] p-7 shadow-2xl text-center">
            ...
            <div className="text-6xl mb-4 animate-bounce">{todayReward.reward}</div>
            <div className="text-2xl font-black text-white mb-1">Racha de {streak + 1} día(s)</div>
            <div className="text-white/50 text-sm mb-6">
              Tu recompensa de hoy: <span className="font-black text-[#FFD700]">{todayReward.label}</span>
            </div>

            <button
              onClick={claim}
              disabled={claiming}
              className="w-full rounded-2xl bg-gradient-to-r from-[#FF5E00] to-[#FF0099] text-white font-black text-base uppercase tracking-wider shadow-[0_0_30px_rgba(255,94,0,0.4)] hover:scale-[1.02] transition-all active:scale-[0.98] py-4 disabled:opacity-60"
            >
              {claiming ? (
