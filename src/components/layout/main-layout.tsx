    ...
    <div className="flex min-h-screen bg-[#0a0a0b] text-white">
      ...
      >
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <Icon size={18} className={isActive ? "text-[#00F0FF]" : ""} />
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          <Link
            href="/support"
            className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-zinc-400 hover:text-white transition-colors"
          >
            <Headphones size={18} /> Soporte
          </Link>
          <div className="text-[10px] text-zinc-600 px-4 pt-2">v1.0.0 • Chido Casino</div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0a0a0b]">
        <header className="h-16 sticky top-0 z-40 glass-panel border-b-0 border-white/5 px-4 lg:px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/wallet"
              className="group flex items-center gap-3 bg-[#1A1A1D] hover:bg-[#222] border border-white/10 rounded-full px-1 pl-4 py-1 transition-all cursor-pointer"
            >
